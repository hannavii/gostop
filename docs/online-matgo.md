...
3인 확장 구조·추가 이벤트·실행 방법은 [온라인 3인 고스톱](./online-gostop.md)을 참고하세요. 현재 로비에서는 2인/3인 모드를 선택할 수 있으며, 아래 문서는 최초 2인 MVP를 설명합니다. 온라인 뻑 소유자는 현재 절대 좌석 번호로 통일되어 있습니다.
...
| 서버 → 클라이언트 | `room:closed` | 퇴장/복구 기한 만료 등 방 종료 알림 |
| 서버 → 본인 | `room:session` | 재접속 토큰 발급 |
| 클라이언트 → 서버 | `room:resume` | 토큰으로 기존 좌석 복구 |

요청은 성공/오류 acknowledgement를 사용합니다. 클라이언트는 연결 및 좌석 복구 후에만 행동을 보내며 응답 대기 중 중복 입력을 막습니다. 게임 행동은 시간 초과 시 자동 재시도하지 않고 상태 새로고침을 안내합니다. 복구 요청의 재시도 정책은 [재접속 문서](./online-reconnect.md)를 참고하세요.
...
- 로그인, DB, 배포, Redis, 관전, 자동 재대결은 포함하지 않습니다. 인메모리 상태는 서버 재시작 시 사라집니다.
- 연결 종료 감지 후 60초 동안 좌석을 보존합니다. 같은 탭의 새로고침/재연결은 sessionStorage의 토큰으로 원래 좌석을 복구합니다. 기한 만료나 명시적 방 나가기는 방을 종료합니다. 자세한 정책은 [재접속 문서](./online-reconnect.md)에 정리했습니다.
